...
class WeekParityTest {
    ...
    @Test
    fun `anchor is a Monday and renders as I`() {
        ...
        assertEquals("I", WeekParity.label(WeekParity.ANCHOR_MONDAY))
        assertEquals(0, WeekParity.ANCHOR_PARITY)
    }
    ...
    fun `every day of a week shares the anchor week's parity`() {
        assertEquals("I", WeekParity.label("2026-08-31"))   // Monday
        for (day in 1..6) assertEquals("I", WeekParity.label("2026-09-0$day"))   // Tue..Sun
        assertEquals("II", WeekParity.label("2026-09-07"))  // next Monday flips
        assertEquals("II", WeekParity.label("2026-09-13"))  // ... through its Sunday
        assertEquals("I", WeekParity.label("2026-09-14"))   // the user's reference week
    }
    ...
    fun `weeks before the anchor alternate backwards`() {
        ...
        assertEquals("II", WeekParity.label("2026-08-24"))
        assertEquals("I", WeekParity.label("2026-08-17"))
    }
    ...
    fun `consecutive weeks alternate across the ISO year boundary`() {
        ...
        assertEquals("II", WeekParity.label("2026-12-28"))   // (0 + 17) % 2 = 1
        assertEquals("I", WeekParity.label("2027-01-04"))    // (0 + 18) % 2 = 0
    }

    @Test
    fun `spring term weeks are still counted from the same anchor`() {
        // 2027-02-01 is 22 weeks after the anchor: (0 + 22) % 2 = 0 -> "I".
        assertEquals(22, WeekParity.weeksBetween("2026-08-31", "2027-02-01"))
        assertEquals("I", WeekParity.label("2027-02-01"))
